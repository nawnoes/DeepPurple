...
with tf.variable_scope("VN"):
    ...
    Flat_W = tf.get_variable("Flat_W", shape=[8 * 8 * 128, 1], initializer=tf.contrib.layers.xavier_initializer())
    Flat_B = tf.get_variable("Flat_B", initializer=tf.random_normal([1], stddev=0.01))
    ...
    # cost = tf.reduce_mean((-(Y+1)/2 * tf.log((hypothesis+1)/2)) - ((1-(Y+1)/2)*tf.log(1-(hypothesis+1)/2)))
    optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(cost)
    # optimizer = tf.train.GradientDescentOptimizer(learning_rate= learning_rate).minimize(cost)
    #Accuracy
    # correct_prediction = tf.equal(tf.argmax(hypothesis,1), tf.argmax(Y,1))
    correct_prediction = (1-tf.abs(Y-tf.nn.sigmoid(hypothesis)))
    # corrrect_prediction =
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(ITERATION):
            ...
            try:
                ...
                trainInput, trainOutput = pgnInput[TRAIN -VALID :TRAIN] ,pgnOutput[TRAIN -VALID:TRAIN] #4000개 학습 데이터
                feed_dict = {X: batch_xs, Y: batch_ys}

                start = time.time()
                s, c, _,h,cp = sess.run([summary,
                                     cost,
                                     optimizer,
                                     hypothesis,
                                     correct_prediction],
                ...
                # for x in range(30):
                #     print("%.3f" % sess.run(tf.nn.sigmoid(h[x]))," ",end="",flush=True)
                # print()
                # print("Y         : ", end="")
                # for x in range(30):
                #     print("%.3f" % batch_ys[x][0]," ", end="", flush=True)
                # print()
                # print("c_p       : ", end="")
                # for x in range(30):
                #     print("%.3f" %cp[x][0]," ",end="",flush=True)
            except:
