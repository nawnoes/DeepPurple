...
with tf.variable_scope("RO"):
    X = tf.placeholder(tf.float32, [None, 8, 8, 16], name="X")  # 체스에서 8X8X10 이미지를 받기 위해 64
    Y = tf.placeholder(tf.float32, [None, 4096], name="Y")

    W1 = tf.get_variable("W1", shape=[3, 3, 16, 128], initializer=tf.contrib.layers.xavier_initializer())
    ...
    W2 = tf.get_variable("W13", shape=[1, 1, 128, 128], initializer=tf.contrib.layers.xavier_initializer())
    B2 = tf.get_variable("B13", initializer=tf.random_normal([128], stddev=0.01))
    L2 = tf.nn.relu(tf.nn.conv2d(L1, W2, strides=[1, 1, 1, 1], padding='SAME') + B2)

    FlatLayer = tf.reshape(L2, [-1, 8 * 8 * 128])
    ...
    for epoch in range(training_epochs):
        ...
        for m, line in enumerate(trainingFile):  # 학습한 이후 부터 다시 시작
            if COUNT <= m and m < COUNT + batchSize:
                ...
                # 입력 데이터 전처리
                batchsizeInput.append(b2a.board2arrayForRollout(board))
