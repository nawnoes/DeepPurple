...
def dividePgnFile(filePath):
    ...
    for i in range(len(fileLinesList)):
        if flag == True:
            currentFilePath = filePath[:-4] + '-%d'%fileCount + filePath[-4:]
        ...
        if "[Event" in fileLinesList[i] and os.path.getsize(currentFilePath) >90000.: #기보의 시작이 [event로 시작 하므로
            ...
            currentFilePath = filePath[:-4] + '-%d'%fileCount + filePath[-4:] #새로 열 파일의 경로를 생성
            fileCount +=1
            tmpFile = open(currentFilePath,"a")
...
def is_LastLearnedFile(path,name):
    file = open(path, "r")
    linesList = file.readlines()  # 파일의 모든 내용을 한 줄씩 List로 받는 함수
